...
#
# Write a function named `calculate_area_triangle` that takes the base and height of a
...
#
# Write a function named `simple_interest` that takes principal, rate of interest (as
# a percentage), and time (years).
# Calculate and return the simple interest using the formula
...
#
# Write a function named `apply_discount` that takes a product's price and a discount
...
# Exercise 5: Sum to N
#
# Write a function named `sum_to` that takes a single integer n and returns the sum
...
#
# Write a function named `largest` that takes three integers as arguments and returns
# the largest of them.
#
# Examples:
...
#
# Create a function called `calculate_tip`. It should take the bill amount and the
# tip percentage (as a whole number).
...
def calculate_tip(bill_amt, tip):
    return (bill_amt * (tip / 100))
...
# 'divide'). 
# Perform the provided operation on the two numbers. In operations where the order of
# numbers is important, 
# treat the first parameter as the first operand and the second parameter as the second
# operand.
#
# Examples:
